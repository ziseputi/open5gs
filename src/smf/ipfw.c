...
int smf_compile_packet_filter(smf_rule_t *smf_rule, char *description)
{
    smf_rule_t zero_rule;
    char *token, *dir;
    char *saveptr;
    int i = 2;
    ...
    memset(smf_rule, 0, sizeof(smf_rule_t));
	for (l = rule->act_ofs, cmd = rule->cmd;
		...
		switch (cmd->opcode) {
        case O_PROTO:
            smf_rule->proto = cmd->arg1;
            break;
        case O_IP_SRC:
        case O_IP_SRC_MASK:
            a = ((ipfw_insn_u32 *)cmd)->d;
            smf_rule->ipv4_local = 1;
            smf_rule->ip.local.addr[0] = a[0];
            if (cmd->opcode == O_IP_SRC_MASK)
                smf_rule->ip.local.mask[0] = a[1];
            else
                smf_rule->ip.local.mask[0] = 0xffffffff;
            break;
        case O_IP_DST:
        case O_IP_DST_MASK:
            a = ((ipfw_insn_u32 *)cmd)->d;
            smf_rule->ipv4_remote = 1;
            smf_rule->ip.remote.addr[0] = a[0];
            if (cmd->opcode == O_IP_DST_MASK)
                smf_rule->ip.remote.mask[0] = a[1];
            else
                smf_rule->ip.remote.mask[0] = 0xffffffff;
            break;
        case O_IP6_SRC:
        case O_IP6_SRC_MASK:
            a = ((ipfw_insn_u32 *)cmd)->d;
            smf_rule->ipv6_local = 1;
            memcpy(smf_rule->ip.local.addr, a, OGS_IPV6_LEN);
            if (cmd->opcode == O_IP6_SRC_MASK)
                memcpy(smf_rule->ip.local.mask, a+4, OGS_IPV6_LEN);
            else
                n2mask((struct in6_addr *)smf_rule->ip.local.mask, 128);
            break;
        case O_IP6_DST:
        case O_IP6_DST_MASK:
            a = ((ipfw_insn_u32 *)cmd)->d;
            smf_rule->ipv6_remote = 1;
            memcpy(smf_rule->ip.remote.addr, a, OGS_IPV6_LEN);
            if (cmd->opcode == O_IP6_DST_MASK)
                memcpy(smf_rule->ip.remote.mask, a+4, OGS_IPV6_LEN);
            else
                n2mask((struct in6_addr *)smf_rule->ip.remote.mask, 128);
            break;
        case O_IP_SRCPORT:
            p = ((ipfw_insn_u16 *)cmd)->ports;
            smf_rule->port.local.low = p[0];
            smf_rule->port.local.high = p[1];
            break;
        case O_IP_DSTPORT:
            p = ((ipfw_insn_u16 *)cmd)->ports;
            smf_rule->port.remote.low = p[0];
            smf_rule->port.remote.high = p[1];
            break;
        }
	}

    memset(&zero_rule, 0, sizeof(smf_rule_t));
    if (memcmp(smf_rule, &zero_rule, sizeof(smf_rule_t)) == 0) {
...
smf_bearer_t *smf_bearer_find_by_packet(ogs_pkbuf_t *pkt)
{
    ...
    if (ip_h->ip_v == 4) {
        ...
        sess = smf_sess_find_by_ipv4(dst_addr[0]);
    } else if (ip_h->ip_v == 6) {
        ip_h = NULL;
    ...
    if (sess) {
        ...
        /* Found */
        ogs_debug("[SMF] Found Session : EBI[%d]", default_bearer->ebi);

        bearer = smf_bearer_next(default_bearer);
        /* Find the bearer with matched */
        for (; bearer; bearer = smf_bearer_next(bearer)) {
            smf_pf_t *pf = NULL;
            ...
            for (pf = smf_pf_first(bearer); pf; pf = smf_pf_next(pf)) {
                int k;
                uint32_t src_mask[4];
                uint32_t dst_mask[4];
